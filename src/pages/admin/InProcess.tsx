...
export default function InProcess() {
  ...
  ], [setDetailRequest, setActionDialog, setNotesDialog, setEditDialog, setDeleteDialog])
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {editDialog && (
        <AdoptionEditDialog request={editDialog} onClose={() => setEditDialog(null)} />
      )}
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          ...
          try {
            await deleteRequest.mutateAsync(deleteDialog.id)
          } finally {
  ...
  return (
    <dialog open className="fixed inset-0 z-50 m-auto w-[min(92vw,40rem)] rounded-2xl border-0 bg-white p-6 shadow-2xl backdrop:bg-black/50 animate-scale-in">
      ...
      <div className="mt-5 flex justify-end gap-3">
        <Button variant="ghost" onClick={onClose} disabled={updateStatus.isPending}>Cancelar</Button>
        <Button onClick={handleSave} isLoading={updateStatus.isPending}>Guardar cambios</Button>
      </div>
    </dialog>
  )
}
